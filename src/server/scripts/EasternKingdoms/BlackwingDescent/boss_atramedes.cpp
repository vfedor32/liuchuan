...
Creature* atramedesShield; // последни?использованный щи?
Creature* roaringsummon;
Creature* _shields[8];

//
// ротация заклинаний пр?наземной фазе:
// pulse, breath, pulse, breath, flame, pulse, breath, pulse
// фаза 80се? значит примерно каждые 13 секунд чт?то должно кастоваться, позж?настро?таймер?точнее
//
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_NEXT_SPELL:
                    //За одну фазу буде?скастовано 7 скилло?                    if (nextspell > 7)
                        break;
                    switch (nextspell)
                    {
                    case 0:
